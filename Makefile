.PHONY: help install install-backend install-frontend build build-backend build-frontend dev dev-backend dev-frontend start start-backend start-frontend clean clean-backend clean-frontend type-check type-check-backend type-check-frontend setup setup-db migrate-db docker-up docker-down docker-logs docker-clean all stop

help:
	@echo "Available commands:"
	@echo "  make setup            - Setup Docker services (PostgreSQL)"
...
dev:
	...
	@echo "Or use: make dev-backend, make dev-frontend in separate terminals"
	@echo ""
