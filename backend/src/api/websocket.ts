...
export class WebSocketHandler {
  ...
  private async handleGameCreated(gameId: string, player1: Player, player2: Player): Promise<void> {
    ...
    if (!game) {
      logger.error('Game not found in handleGameCreated', { gameId, player1Id: player1.id, player2Id: player2.id });
      return;
    }
    ...
    for (const [socketId, socketPlayer] of this.socketPlayers.entries()) {
      if (socketPlayer.player.id === player1.id || socketPlayer.player.id === player2.id) {
        ...
        if (socket) {
          ...
          logger.warn('Socket not found for player in game creation', { socketId, playerId: socketPlayer.player.id });
        }
      }
    }
    ...
    if (playersFound < 2) {
      logger.warn('Not all players found when creating game', { 
        ...
        playersFound,
        allSocketPlayers: Array.from(this.socketPlayers.entries()).map(([sid, sp]) => ({
          socketId: sid,
    ...
    logger.info('Game created and players notified', { gameId, player1Id: player1.id, player2Id: player2.id, playersFound });
  }
  ...
  private async handleJoinGame(socket: any, data: { username: string; gameId?: string }): Promise<void> {
    try {
      ...
      if (gameId) {
        ...
        if (!game) {
          logger.warn('Game not found immediately, checking matchmaking queue', { 
            gameId, 
            username, 
            socketId: socket.id
          });
          
          const waitingGame = this.matchmakingService.findWaitingGameByGameId(gameId);
          ...
          if (waitingGame) {
            ...
            if (this.isUsernameTaken(trimmedUsername, socket.id)) {
              ...
              const joiningPlayer = new Player(uuidv4(), availableUsername, 'human');
              this.activeUsernames.set(availableUsername, socket.id);
              
              try {
                await this.databaseService.savePlayer(joiningPlayer.id, availableUsername);
              } catch (dbError) {
                logger.warn('Failed to save joining player to database', { error: dbError });
              }
            ...
            const joiningPlayer = new Player(uuidv4(), trimmedUsername, 'human');
            this.activeUsernames.set(trimmedUsername, socket.id);
            
            try {
              await this.databaseService.savePlayer(joiningPlayer.id, trimmedUsername);
            } catch (dbError) {
              logger.warn('Failed to save joining player to database', { error: dbError });
            }
        ...
        if (!game) {
          logger.error('Game not found when joining with gameId after retry', { 
            ...
            username, 
            socketId: socket.id,
            activeGameIds: Array.from(this.gameStateManager.getAllActiveGames().map(g => g.id))
          });
          socket.emit('error', { message: 'Game not found. It may have expired or does not exist.' });
          return;
        }

        let player = game.player1.username === username
          ? game.player1
        ...
        const trimmedUsername = username.trim();
        if (this.isUsernameTaken(trimmedUsername, socket.id) && player.username === trimmedUsername) {
          const availableUsername = this.generateUniqueUsername(trimmedUsername);
        ...
        if (existingSocketPlayer) {
          if (existingSocketPlayer.player && existingSocketPlayer.player.username) {
      ...
      try {
        ...
        logger.warn('Failed to save player to database, continuing anyway', { error: dbError, playerId, username: finalUsername });
      }
      ...
      socket.emit('waiting-for-opponent', { playerId: player.id, username: finalUsername });
      logger.info('Player joined matchmaking', { socketId: socket.id, playerId: player.id, username: finalUsername });
    } catch (error: any) {
      logger.error('Error handling join-game', { error, socketId: socket.id, errorMessage: error?.message });
      socket.emit('error', { message: error?.message || 'Failed to join game. Please try again.' });
    }
  }

  private async handleCreateGame(socket: any, data: { username: string }): Promise<void> {
    try {
      const { username } = data;
      ...
      logger.info('Game room created, waiting for opponent', { gameId, playerId: player.id, username: finalUsername });
      ...
      this.matchmakingService.addPlayer(player, gameId, async (matchedPlayer: Player) => {
      ...
      logger.error('Error handling create-game', { error, socketId: socket.id, errorMessage: error?.message });
      socket.emit('error', { message: error?.message || 'Failed to create game. Please try again.' });
    }
  }
  ...
  private async handleMakeMove(socket: any, data: { gameId: string; column: number }): Promise<void> {
    try {
      ...
      if (!socketPlayer) {
        logger.warn('Make move attempted without authentication', { socketId: socket.id, gameId });
      ...
      if (!game && socketPlayer.gameId) {
        logger.warn('Game not found with provided gameId, trying socketPlayer gameId', { 
      ...
      logger.debug('Make move request', { 
        ...
        playerId: socketPlayer.player.id,
        socketPlayerGameId: socketPlayer.gameId,
        allGameIds: Array.from(this.gameStateManager.getAllActiveGames().map(g => g.id))
      });
      ...
      if (!game) {
        ...
        const playerGame = this.gameStateManager.findGameByPlayer(socketPlayer.player.id);
        if (playerGame) {
          game = playerGame;
          gameId = playerGame.id;
          socketPlayer.gameId = gameId;
          logger.info('Found game by player ID', { gameId, playerId: socketPlayer.player.id });
        }
      }
      ...
      if (gameStatus === 'won' || gameStatus === 'draw') {
        logger.info('Game ended after move', { gameId, status: gameStatus, winner: game.getWinner()?.username || null });
        setTimeout(async () => {
      ...
      logger.info('Move made', { gameId, playerId: socketPlayer.player.id, column, status: gameStatus });
    } catch (error: any) {
  ...
  private async handleBotMove(gameId: string): Promise<void> {
    ...
    try {
      ...
      logger.error('Error handling bot move', { error, gameId, errorMessage: error?.message });
      const game = this.gameStateManager.getGame(gameId);
      if (game && game.getStatus() === 'playing') {
        ...
        if (currentPlayer.isBot()) {
          ...
          try {
            const availableColumns = game.getEngine().getBoard().getAvailableColumns();
            ...
            logger.error('Fallback bot move also failed', { error: fallbackError, gameId });
          }
        }
      }
    }
  ...
  private handleDisconnect(socket: any): void {
    ...
    if (socketPlayer.player && socketPlayer.player.username) {
      ...
      if (existingSocketId === socket.id) {
        ...
        logger.debug('Username freed on disconnect', { username, socketId: socket.id });
      }
    }
    ...
    logger.info('Player disconnected', { socketId: socket.id, playerId: socketPlayer.player.id, username: socketPlayer.player.username });
  }
