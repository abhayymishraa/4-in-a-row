...
export class Game {
  ...
  makeMove(column: number, playerId: string): void {
    if (!this.isPlayerTurn(playerId)) {
      throw new Error(
        `Not player's turn. Current player: ${this.getCurrentPlayer().id}`,
      );
    }
    ...
    if (moveResult.status === "won" && moveResult.winner) {
      ...
      if (engineWinner !== moveResult.winner) {
        throw new Error(
          `Win detection mismatch: moveResult.winner=${moveResult.winner}, engine.getWinner()=${engineWinner}`,
        );
      }
      ...
      if (this.engine.getGameStatus() !== "draw") {
        throw new Error(
          `Draw detection mismatch: moveResult.status=draw but engine.getGameStatus()=${this.engine.getGameStatus()}`,
        );
      }
    }
  }
