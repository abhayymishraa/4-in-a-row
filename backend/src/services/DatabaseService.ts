...
export class DatabaseService {
  ...
  constructor() {
    ...
    if (process.env.DB_SSL === "true" || process.env.DB_SSL === "required") {
      ...
      const isCloudHost =
        cloudHosts.some((host) => dbHost.includes(host)) || isProduction;
    ...
    try {
      ...
      `,
        ...
        .catch((error: any) => {
          if (error.code !== "42704" && error.code !== "42804") {
            logger.warn(
              "Could not alter games table columns (may already be nullable or column not found):",
              { error: error?.message },
            );
          }
        });
      ...
      `,
        ...
        .catch((error: any) => {
          if (error.code !== "42704" && error.code !== "42804") {
            logger.warn(
              "Could not alter game_moves table columns (may already be nullable or column not found):",
              { error: error?.message },
            );
          }
        });
      ...
      logger.error(
        "All database operations (savePlayer, saveGame, getLeaderboard) will be DISABLED",
      );
      throw error;
    }
  }
  ...
  async savePlayer(playerId: string, username: string): Promise<string> {
    if (!this._isInitialized) {
      logger.warn("SKIPPING savePlayer - Database not initialized", {
        playerId,
        username,
        host: process.env.DB_HOST || "NOT SET",
        database: process.env.DB_NAME || "NOT SET",
    ...
    logger.debug("Attempting to save player to database", {
      playerId,
      username,
    });
    ...
    try {
      ...
      if (result.rows.length === 0) {
        ...
        if (recheck.rows.length > 0) {
          logger.debug(
            "Player was inserted by another process, using existing ID",
            {
      ...
      logger.info("Player saved to database successfully", {
        playerId: savedId,
        username,
      });
      return savedId;
      ...
      if (error.code === "23505") {
        logger.debug(
          "Player already exists (unique constraint), fetching existing ID",
          { playerId, username },
        );
  ...
  async saveGame(game: Game): Promise<void> {
    if (!this._isInitialized) {
      logger.warn("SKIPPING saveGame - Database not initialized", {
        gameId: game.id,
        host: process.env.DB_HOST || "NOT SET",
        database: process.env.DB_NAME || "NOT SET",
    ...
    try {
      ...
      if (game.player1.isHuman()) {
        try {
          dbPlayer1Id = await this.savePlayer(
            game.player1.id,
            game.player1.username,
          );
          logger.debug("Player1 saved to database", {
            playerId: dbPlayer1Id,
            username: game.player1.username,
          });
        } catch (error: any) {
          logger.warn("Failed to save player1, continuing", {
            error: error?.message,
      ...
      if (game.player2.isHuman()) {
        try {
          dbPlayer2Id = await this.savePlayer(
            game.player2.id,
            game.player2.username,
          );
          logger.debug("Player2 saved to database", {
            playerId: dbPlayer2Id,
            username: game.player2.username,
          });
        } catch (error: any) {
          logger.warn("Failed to save player2, continuing", {
            error: error?.message,
      ...
      const status =
        game.getStatus() === "won" ? "completed" : game.getStatus();
      ...
      if (dbPlayer1Id || dbPlayer2Id) {
        ...
        if (finalWinnerId) {
          ...
          if (updateResult.rows.length > 0) {
            ...
            logger.warn("Winner ID not found when updating games_won", {
              winnerId: finalWinnerId,
            });
          }
        }
      } else {
        logger.debug("Skipping game save - both players are bots", {
          gameId: game.id,
        });
      }
      ...
      if (
        ...
        logger.error(
          "Database connection error - check DB_HOST, DB_PORT, and network connectivity",
        );
