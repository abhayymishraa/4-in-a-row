...
const GameStatus = ({ game, currentUsername }: GameStatusProps) => {
  ...
  return (
    <div className="mb-5 p-3 bg-white rounded-lg shadow-sm">
      <div className="mb-2">
        <strong>Player 1:</strong> {game.player1.username}{" "}
        {game.player1.type === "bot" && "(Bot)"}
      </div>
      <div className="mb-2">
        <strong>Player 2:</strong> {game.player2.username}{" "}
        {game.player2.type === "bot" && "(Bot)"}
      </div>
      <div
        className={`mt-3 text-lg font-bold ${isGameOver ? "text-green-500" : "text-blue-500"}`}
      >
        {statusMessage}
      </div>
    </div>
