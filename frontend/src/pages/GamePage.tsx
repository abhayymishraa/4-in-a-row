  ...
  useEffect(() => {
    ...
    socket.on("error", (data: { message: string }) => {
      ...
      if (
        ...
        data.message.includes("not a player")
      ) {
        ...
        {botCountdown !== null && botCountdown > 0 && (
          <div className="p-5 bg-blue-50 text-blue-800 mt-5 rounded-lg border border-blue-200">
            ...
            <p className="mt-3 text-sm text-blue-700 italic">
              If no one joins in{" "}
              <span className="font-bold">{botCountdown}</span> second
              {botCountdown !== 1 ? "s" : ""}, a bot will automatically join
            </p>
          </div>
        )}
        {botCountdown === 0 && (
  ...
  return (
    <div className="p-5 max-w-4xl mx-auto">
      ...
      {botCountdown !== null &&
        botCountdown > 0 &&
        game &&
        game.player2?.type === "bot" && (
          <div className="p-4 bg-orange-50 text-orange-700 mb-5 rounded-lg text-center">
            <p className="mb-1 font-bold">Waiting for opponent...</p>
            <p>
              If no participant joins, a bot will automatically join in{" "}
              <strong>{botCountdown}</strong> second
              {botCountdown !== 1 ? "s" : ""}.
            </p>
          </div>
        )}
      ...
      {game && (game.status === "won" || game.status === "draw") && (
        <div
          className={`p-5 mb-5 rounded-lg text-center text-2xl font-bold ${
            game.status === "won"
